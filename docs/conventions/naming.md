...
Rule IDはREADMEで定義された `NM-<Scope>-<Category>-<Number>` 形式に従う。本Conventionで新規定義するRuleのScopeには`BS`を使用する。

---

## File Naming（ファイル命名）

### NM-BS-FN-001: Lowercase File Names
...
### NM-BS-FN-003: Meaningful File Names
...
### NM-BS-FN-004: Stable File Names
...
### NM-BS-DN-001: Lowercase Directory Names
...
NM-BS-FN-001と同じ理由による。ファイル名とディレクトリ名の命名規則を統一することで、パス全体の一貫性を保つ。
...
### NM-BS-DN-002: Responsibility-Based Directory Names
...
### NM-BS-DN-003: Stable Directory Names
...
### NM-BS-DC-001: English Base Title
...
英語タイトルは機械的処理主体による識別・検索・参照に適している。日本語補足が必要な場合はWR-BS-LG-003に従い括弧書きで付記する。
...
### NM-BS-DC-002: Title Matches File Responsibility
...
### NM-BS-ID-001: Rule ID Format

| 項目 | 内容 |
| --- | --- |
| Rule | Rule IDはConventions READMEで定義された `<Convention>-<Scope>-<Category>-<Number>` 形式に従う |
| Requirement | MUST |
...
```text
良い例: MD-BS-DS-001、WR-BS-LG-001、NM-BS-FN-001
避ける例: MD001、MarkdownRule1、rule-ds-001
```

---

### NM-BS-ID-002: Sequential Rule Numbering

| 項目 | 内容 |
| --- | --- |
| Rule | Rule NumberはConvention・Scope・Categoryの組み合わせごとに、ゼロパディングした3桁で採番する |
| Requirement | MUST |
| Source | Project Conventions |
...
```text
良い例: MD-BS-DS-001、MD-BS-DS-002、MD-BS-DS-010
避ける例: MD-BS-DS-1、MD-BS-DS-2、MD-BS-DS-10
```

---

### NM-BS-ID-003: Stable Rule IDs
...
### NM-BS-ID-004: ADR Sequential Numbering
...
### NM-BS-GH-001: Branch Name Format
...
- [ ] Rule IDは `<Convention>-<Scope>-<Category>-<Number>` 形式に従っているか
