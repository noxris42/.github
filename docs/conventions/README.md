...
### Rule ID Components（Rule IDの構成要素）

| 要素 | 意味 |
| --- | --- |
| Convention | Conventionを示す2文字識別子 |
| Scope | Ruleを最初に定義したレイヤーまたはPatternを示す2文字識別子 |
| Category | Convention内のRule Categoryを示す2文字識別子 |
| Number | 同一Convention・Scope・Category内の3桁連番 |

### Scope Identifier（Scope識別子）

本書では、Conventions全体で共通して使用する基本Scopeのみを定義する。

| Scope | 意味 |
| --- | --- |
| `BS` | Base Conventionで定義されたRule |
| `RP` | Repository Conventionで新規にAddされたRule |

Pattern固有のScopeは各Pattern Conventionまたは上位のConvention Extension Architectureで管理し、本書にはPatternコード一覧を複製しない。

- Pattern Conventionで新規にAddするRuleは、そのPattern固有のScopeを使用する
- Replace・Extend・Disableでは、既存Rule IDをそのまま維持する
- Scopeは、現在Ruleが記載されている文書ではなく、Ruleを最初に定義したレイヤーを示す
...
CategoryおよびNumberの採番方法は各Convention文書で定義する。採番単位はConvention・Scope・Categoryの組み合わせとする。既存のBase Ruleは、Category・Numberを変更せず`BS`を付加する形で移行する。一度使用したRule IDは再利用せず、欠番は許容する。

本書ではRule ID構造のみを管理する。Convention拡張時の具体的な操作・記述形式は、上位のConvention Extension Architecture（`docs/architecture/convention-extension.md`）を参照する。
...
コミットメッセージの構造・記述方法・運用ルールを定義する。

実装: `commit.md`

### Branch Convention（ブランチ規約）

ブランチの命名規則・ライフサイクル・作成・削除タイミングを定義する。

実装: `branch.md`

### GitHub Project Convention（GitHubプロジェクト運用規約）

GitHub Projectsの日常運用ルール（Status変更タイミング・Review・Blocked・Done条件）を定義する。

実装: `github-project.md`
...
### Convention Does Not Reference Specifications（ConventionはSpecificationを参照しない）

Convention文書は、同一Convention層または上位設計のみを参照する。Specification・Templateなど下位レイヤーへの依存を持たない。SpecificationはConventionを参照・利用する立場にある。
