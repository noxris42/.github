...
Rule IDはREADMEで定義された `CM-<Scope>-<Category>-<Number>` 形式に従う。本Conventionで新規定義するRuleのScopeには`BS`を使用する。

---

## Message Structure（メッセージ構造）

### CM-BS-MS-001: Message Format（メッセージ形式）
...
### CM-BS-MS-002: Section Separation（セクション区切り）
...
### CM-BS-MS-003: Required Sections（必須セクション）
...
### CM-BS-LG-001: Language Selection（言語選択）

| 項目 | 内容 |
| --- | --- |
| Rule | コミットメッセージの言語は対象読者およびRepository Profileに応じて選択する |
| Requirement | MUST |
| Source | Writing Convention (WR-BS-LG-001), Project Conventions |

**Reason:**

コミット履歴の可読性は対象読者が理解できる言語で最大化されるため。言語の選択はWR-BS-LG-001の原則に従い、Repository Overrideで明示する。

---

## Type & Gitmoji（タイプとGitmoji）

### CM-BS-TG-001: Type Required（Type必須）
...
### CM-BS-TG-002: Common Type Candidates（共通Type候補）
...
### CM-BS-TG-003: Gitmoji Placement（Gitmoji配置）
...
### CM-BS-TG-004: Gitmoji-Type One-to-One（Gitmoji-Type 1対1対応）
...
### CM-BS-SC-001: Scope Optional（Scope任意）
...
### CM-BS-SC-002: Scope Format（Scope形式）
...
### CM-BS-SC-003: Scope Definition（Scope定義）
...
### CM-BS-SB-001: Subject Required（Subject必須）
...
### CM-BS-SB-002: Subject Content（Subjectの内容）
...
### CM-BS-SB-003: Subject Length（Subject長さ）
...
### CM-BS-SB-004: Subject Imperative（Subject命令形）

| 項目 | 内容 |
| --- | --- |
| Rule | 英語で記述する場合、Subjectは動詞の原形（命令形）で始める |
| Requirement | SHOULD |
| Source | Writing Convention (WR-BS-WS-002), Project Conventions |
...
### CM-BS-BD-001: Body Optional（Body省略可）
...
### CM-BS-BD-002: Body Content（Bodyの内容）
...
### CM-BS-FT-001: Footer Optional（Footer省略可）
...
### CM-BS-FT-002: Issue Reference（Issue参照）
...
### CM-BS-FT-003: No Closing Keywords（クローズキーワード禁止）
...
### CM-BS-FT-004: Breaking Change（破壊的変更）
...
### CM-BS-TC-001: Footer Key-Value Format（Footerのキーバリュー形式）
...
### CM-BS-TC-002: No Ambiguous Keys（曖昧なキー禁止）
...
### CM-BS-RO-001: Override Scope（上書き可能範囲）
...
### CM-BS-RO-002: Override Documentation（上書きの明文化）
...
- [ ] GitmojiとTypeはCM-BS-TG-002の対応表に従い1対1で対応しているか
