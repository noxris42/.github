...
`docs/conventions/README.md`および既存Base Conventionは、本Specificationが定義する4要素形式（`<Convention>-<Scope>-<Category>-<Number>`）を使用している。既存のBase Ruleは、いずれも`BS` Scopeを持つ。

本SpecificationとConventions READMEのRule ID構造は整合しており、旧3要素形式（`<Convention>-<Category>-<Number>`）との互換処理は提供しない。
...
Rule ID移行が完了したことにより、以下の例で使用するBase Rule ID（例：`BR-BS-NM-001`）は、`docs/conventions/branch.md`に実在するRule IDおよびRule内容と一致する。ただし、Pattern Convention（`main-only`・`branch-required`・`branch-default`）自体は本書執筆時点で未実装であるため、Pattern ConventionによるReplace・Extend・Add、およびPattern Convention経由の操作を示す例は、引き続き概念例として扱う。
