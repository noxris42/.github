...
- Branch命名規則の詳細 → `docs/conventions/branch.md`
...
BranchはIssueを実現するための作業空間である。Branchの作成・対応関係は `docs/conventions/branch.md` に従う。ブランチの命名規則は `docs/conventions/branch.md` を参照する。
...
MergeはIssueに対応する作業完了の確定点である。Merge後のBranch削除は `docs/conventions/branch.md` に従う。
