...
その内部でRule（ルール）がどのような意味構造を持つのかを定義する
上位Modelである。
...
4. Normative Rule（規範的ルール）はどのように
   **安定して参照され続けるのか**
   （Rule Identity／ルール同一性、およびその安定性）。

本文書は上位Architectureとして自己完結する。
個別のConvention（規約）の内容、およびConvention（規約）の
具体的な記述方法（Convention Authoring Rule／規約記述ルール）の
存在や内容を前提としない。
...
Ownership（所有責任）・Foundation Application（共通開発基盤を対象Repositoryへ
適用する関係）・Shared Scope（共有範囲）の判断を
再定義・上書きしない。
...
- Markdown等、特定の記述媒体上でのRule（ルール）の表現方法
- Rule（ルール）のCategory（分類）やSection配置の方法
- 個別Convention（規約）の内容
- Convention（規約）を代表・集約する文書の要否と責務
- Convention Application（規約適用）の詳細、および
  Override（上書き）／Extend（拡張）／Replace（置換）／Disable（無効化）
- Metadata（構造化メタデータ）およびDeclaration（明示的宣言）のSchema
- Release（リリース）およびVersioning（版管理）の具体実装
...
Convention（規約）
  │
  ├─ defines ─▶ Normative Rule（規範的ルール）── Normative Effect（規範的効力）を持つ
  │                  │
  │                  └─ identified by ─▶ Rule Identity（ルール同一性）
  │
  └─ may contain ─▶ Non-normative Content（非規範的内容）
                    （説明・Example・補足等／
                      それ自体では規範要求を追加しない）
```
...
| 要素 | 意味 |
| --- | --- |
| Rule Identity（ルール同一性） | そのRule（ルール）をStable Reference（安定参照）するための同一性 |
| Rule Name（ルール名） | そのRule（ルール）を人間が識別・想起するためのLabel（ラベル） |
| Requirement Level（要求レベル） | そのRule（ルール）が要求・禁止・推奨・許容のいずれであるかの水準 |
| Rule Statement（ルール文） | そのRule（ルール）が対象に対して規定する内容そのもの |
| Reason（理由） | そのRule（ルール）が必要である目的・問題・制約 |

### Optional Elements（任意要素）

| 要素 | 意味 |
| --- | --- |
| Supplementary Information（補足情報） | 理解を助ける説明・Example（例）・注意点等 |

Supplementary Information（補足情報）は
Non-normative Content（非規範的内容）であり、
それ自体では新たなNormative Requirement（規範要求）を追加しない。

### Reason（理由）の要件

Reason（理由）は、Rule Statement（ルール文）の
**単なる言い換えであってはならない** 。

Reason（理由）が説明するのは、
そのRule（ルール）が **なぜ必要なのか** 、
すなわち達成しようとする目的、回避しようとする問題、
前提となる制約である。

Reason（理由）を必須とするのは、次のためである。

- Rule（ルール）の妥当性を、後から検証・再評価できるようにする。
- Requirement Level（要求レベル）が `SHOULD` / `SHOULD NOT` の場合に、
  例外を認めてよいかを判断できるようにする。
- Rule（ルール）の変更・廃止が妥当かを、目的に照らして判断できるようにする。

### Rule Name（ルール名）とRule Identity（ルール同一性）の区別

Rule Name（ルール名）は
**Human-readable Label（人間向けラベル）** であり、
...
外部のRFC等に対するNormative Dependency（規範的依存）は設けない。
すなわち、外部文書の改訂や解釈が
...
| 構成要素 | 意味 |
| --- | --- |
| Convention Identity（規約同一性） | そのRule（ルール）がどのConvention（規約）に属するか |
| Definition Namespace（定義名前空間） | そのRule（ルール）がどの定義名前空間に属するか |
| Rule-local Identity（ルール内同一性） | 同一のConvention Identity・Definition Namespaceの中で、そのRule（ルール）を一意に識別する同一性 |
...
したがって、Namespace（名前空間）から次を推論してはならない。
...
Rule（ルール）間に何らかの関係が必要となる場合、
その関係はNamespace（名前空間）から暗黙に導出されるのではなく、
後続設計において明示的に定義される。

## Rule Identity Stability（ルール同一性の安定性）

Rule Identity（ルール同一性）の安定性は、
**Development Version（開発版）とStable Version（安定版）の境界** で扱う。

本文書は境界の意味のみを定義し、
Development Version（開発版）／Stable Version（安定版）の
具体的なRelease Management（リリース管理）方法は定義しない。

### Development Version（開発版）における扱い

- Development Version（開発版）で新規に導入された
  Rule Identity（ルール同一性）は、
  Stable Release（安定版リリース）前であれば、
  設計変更に伴って再編できる。
- 過去のStable Version（安定版）ですでに公開された
  Rule Identity（ルール同一性）は、
  Development Version（開発版）の途中であっても変更しない。

### Stable Version（安定版）における扱い

- Stable Release（安定版リリース）された
...
| 変更の性質 | 扱い |
| --- | --- |
| Normative Meaning（規範的意味）が維持される変更（表現の改善、Rule Name（ルール名）の変更、Reason（理由）や補足の拡充等） | 既存のRule Identity（ルール同一性）を維持する |
| Normative Meaning（規範的意味）が実質的に変わる変更 | 新しいRule Identity（ルール同一性）とする |
| Split（分割）またはMerge（統合）によって新しいNormative Rule（規範的ルール）が成立する場合 | 新しいRule Identity（ルール同一性）とする |
...
Normative Rule（規範的ルール）はReason（理由）を必須要素として持つ。
理由を伴わないRule（ルール）は、検証も再評価もできない。
...
外部文書へのNormative Dependency（規範的依存）を設けない。
...
- Rule Identity（ルール同一性）に含まれるNumber（番号）の桁数・採番方法
- Markdown上でのRule（ルール）の表現方法
  （見出しの取り方、Rule Name（ルール名）の併記形、項目名の並べ方等）
- Rule Model（ルールモデル）の各要素を文書上でどう記述単位へ対応させるか
- Rule（ルール）のCategory（分類）およびSection配置の方法

### Conformance（適合）に関する事項

本文書は、Normative Rule（規範的ルール）のみが
Normative Effect（規範的効力）を持つという境界のみを定めており、
次は定義しない。

- Convention（規約）全体としてのConformance Model（適合モデル）
- 適合・不適合の判定方法
...
これはDesign Gap（設計不足）ではなく、
現時点で確定していない事項を先行決定しないための
意図的な委譲である。

### Convention Extension（規約拡張）に関する事項

- Convention Extension（規約拡張）における拡張単位
...
Foundation Application（共通開発基盤を対象Repositoryへ適用する関係）が扱う。
...
### Release / Versioningに関する事項

- Development Version（開発版）／Stable Version（安定版）の
  具体的なRelease Management（リリース管理）方法
- Version表記およびVersioning規則

## Usage by Downstream Design（下位設計からの参照）

個別のConvention（規約）および
Convention Authoring Rule（規約記述ルール）は、
本文書を参照して次を前提にできる。
これらを再定義する必要はない。
