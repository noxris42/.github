...
設計検討（Design Exploration）を記録する。

扱うもの: 設計背景・設計検討・比較した案・議論の流れ・判断候補・未採用案

扱わないもの: 現行仕様・正式なArchitecture Decision・運用ルール
...
| 何を検討したか（設計探索の過程） | design-records/ |
...
philosophy・architecture・conventions・specifications・design-records・adrの各責務領域は、互いの責務を侵食しない。
